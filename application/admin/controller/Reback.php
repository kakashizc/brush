 ...
 * @icon fa fa-circle-o
 */
class Reback extends Backend
{
    
    /**
     * Reback模型对象
     * @var \app\admin\model\Reback
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Reback;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                ...
                $row->visible(['id','money','images','createtime','status','extra']);
                $row->visible(['admin']);
        ...
        if ($this->request->isPost()) {
            ...
            if ( $params ) {
                ...
                try {
                    ...
                    if($row->status != '1'){//如果状态不是1,说明已经审核过了,直接跳出
                        Db::rollback();
                        $this->error('不可重复审核');
                    }
